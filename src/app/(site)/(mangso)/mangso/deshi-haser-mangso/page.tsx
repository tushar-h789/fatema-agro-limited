...
export default function GurProducts() {
  ...
  const subCategoryId = SubCategoryId.Duck;

  const haserMangso = getProductsBySubCategory(subCategoryId);

  if (haserMangso.length === 0) {
    return (
    ...
    <section className="bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          দেশি হাঁসের মাংস (Deshi Duck Meat)
          </h2>
        </div>

        {/* Products Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {haserMangso.map((product) => (
            <div
              key={product.id}
