...
export default function GurProducts() {
  ...
  const subCategoryId = SubCategoryId.Cow;

  const goruMangso = getProductsBySubCategory(subCategoryId);

  if (goruMangso.length === 0) {
    return (
    ...
    <section className="bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          দেশি গরুর মাংস  (Cow Meat)
          </h2>
        </div>

        {/* Products Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {goruMangso.map((product) => (
            <div
              key={product.id}
