...
export default function GurProducts() {
  ...
  const subCategoryId = SubCategoryId.Goat;

  const chagolerMangso = getProductsBySubCategory(subCategoryId);

  if (chagolerMangso.length === 0) {
    return (
  ...
  return (
    <section className="bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-center mb-8 text-2xl font-bold text-slate-100 py-10 bg-gradient-to-r from-green-700  bg-opacity-40">
          দেশি ছাগলের মাংস  (Deshi Goat Meat)
          </h2>
        </div>

        {/* Products Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {chagolerMangso.map((product) => (
            <div
              key={product.id}
