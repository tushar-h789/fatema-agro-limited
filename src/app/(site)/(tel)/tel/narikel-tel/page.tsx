...
export default function NarikelTel() {
  const router = useRouter();
  const subCategoryId = SubCategoryId.CoconutOil;

  const NarikelTel = getProductsBySubCategory(subCategoryId);

  if (NarikelTel.length === 0) {
    return (
  ...
  const handleCardClick = (id: string) => {
    router.push(`/tel/narikel-tel/${id}`);
  };
